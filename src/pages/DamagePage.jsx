...
export const DamagePage = () => {
  ...
  const sampleImages = [
    { title: 'Tsunami Surge Wave', url: 'https://images.unsplash.com/photo-1505118380757-91f5f5632de0?auto=format&fit=crop&w=600&q=80' },
    { title: 'Landslide Slope Failure', url: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?auto=format&fit=crop&w=600&q=80' },
    { title: 'Cyclone Wind Shear', url: 'https://images.unsplash.com/photo-1527482797697-8795b05a13fe?auto=format&fit=crop&w=600&q=80' },
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-black text-white">MODULE 7: AI VISION DAMAGE ASSESSMENT</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-lg bg-slate-900 border border-amber-500/40 rounded-2xl p-6 shadow-2xl space-y-4">
            <h3 className="font-extrabold text-base text-white flex items-center gap-2">
              <FileWarning className="w-5 h-5 text-amber-400" /> Upload Infrastructure Damage Evidence
            </h3>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-300 uppercase mb-1">Disaster Category</label>
                <select
                  value={disasterType}
                  onChange={(e) => setDisasterType(e.target.value)}
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs font-semibold text-white"
                >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 uppercase mb-1">Location of Damaged Site</label>
                <input
                  type="text"
                  required
                  value={location}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 uppercase mb-1">Select Preset Evidence Photo URL</label>
                  ...
                  {sampleImages.map((img, i) => (
                    <button
                      ...
                      className={`p-1 rounded-xl border text-left text-[10px] ${
                        imageUrl === img.url ? 'border-amber-400 bg-amber-950/40 text-amber-300 font-bold' : 'border-slate-800 text-slate-400'
                      }`}
                    >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 uppercase mb-1">Assess Severity (or let AI evaluate)</label>
                <select
                  value={damageLevel}
                  onChange={(e) => setDamageLevel(e.target.value)}
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs font-bold text-amber-400"
                >
                  <option value="High">HIGH DAMAGE (Critical Foundation Loss)</option>
                  <option value="Medium">MEDIUM DAMAGE (Impaired Transport/Grid)</option>
                  <option value="Low">LOW DAMAGE (Debris / Minor Infiltration)</option>
                </select>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {damageReports.map(dmg => (
          <div key={dmg.id} className="glass-panel glass-panel-hover p-5 rounded-2xl border border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <span className="font-black text-xs text-amber-400">{dmg.id} • {dmg.disaster_type}</span>
            ...
            <div>
              <h3 className="font-extrabold text-sm text-white flex items-center gap-1.5">
                <MapPin className="w-4 h-4 text-cyan-400" /> {dmg.location}
              </h3>
            ...
            <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 text-xs text-slate-300 space-y-1">
              <div className="font-bold text-amber-400 flex items-center gap-1 text-[11px]">
                <Sparkles className="w-3.5 h-3.5 text-amber-400" /> AI Vision Inspection Report:
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">{dmg.summary}</p>
            </div>
          </div>
        ))}
      </div>
