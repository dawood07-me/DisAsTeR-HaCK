...
export const AuthPage = () => {
  ...
  const renderEntryPage = () => (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-3">
        ...
        <h1 className="text-4xl font-black tracking-wider text-slate-900 dark:text-white">
          Raksh<span className="text-cyan-600 dark:text-cyan-400">AI</span>
        </h1>
        <p className="text-sm text-slate-600 dark:text-slate-400 font-medium max-w-xs mx-auto leading-relaxed">
      ...
      {/* Footer */}
      <p className="text-center text-[10px] text-slate-500 dark:text-slate-600 font-medium">
  ...
  const renderRoleSelection = () => (
    <div className="space-y-6">
      <button onClick={() => changeStep(STEP_ENTRY)} className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 font-bold transition">
        <ChevronLeft className="w-4 h-4" /> Back
      </button>

      <div className="text-center space-y-2">
        <h2 className="text-2xl font-black text-slate-900 dark:text-white">Who are you?</h2>
        <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">Select your role to continue</p>
      </div>
            ...
            <button
              ...
              className={`p-4 rounded-2xl border text-left transition-all group ${
                selected
                  ...
                  : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/60 hover:bg-slate-200 dark:hover:bg-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700'
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                ...
                {selected && <CheckCircle2 className="w-5 h-5 text-cyan-600 dark:text-cyan-400" />}
              </div>
              <span className="font-extrabold text-sm text-slate-900 dark:text-white block">{role.label}</span>
              <span className="text-[11px] text-slate-600 dark:text-slate-400 mt-1 block leading-relaxed">{role.desc}</span>
            </button>
          );
        })}
      </div>
  ...
  const renderDemoSelect = () => (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <h2 className="text-2xl font-black text-slate-900 dark:text-white">🚀 Demo Mode</h2>
        <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">Quick access with pre-verified accounts</p>
      </div>
        ...
        {DEMO_ACCOUNTS.map(account => (
          ...
          >
            ...
            <div className="text-left flex-1">
              ...
              <span className="block text-[10px] font-medium opacity-90">{account.email}</span>
            </div>
      ...
      <div className="p-3 rounded-xl bg-purple-100/70 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800/40 text-center">
        <p className="text-[11px] text-purple-900 dark:text-purple-300 font-bold">
          🔐 All demo accounts are pre-verified with <span className="font-black">password: 123456</span>
        </p>
      </div>
    </div>
  );
  ...
  const renderDemoVerify = () => (
    <div className="space-y-6">
      <div className="text-center space-y-3">
        ...
        {demoAccount && (
          <p className="text-xs text-slate-700 dark:text-slate-300 font-extrabold flex items-center justify-center gap-1.5">
            <span>{demoAccount.emoji}</span>
            <span>{demoAccount.role}</span>
            <span className="text-slate-400">•</span>
            <span className="text-slate-600 dark:text-slate-400 font-semibold">{demoAccount.email}</span>
          </p>
        )}
      </div>
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 p-3.5 rounded-2xl border transition-all duration-500 ${
                isComplete
                  ? 'bg-emerald-50/90 dark:bg-emerald-950/40 border-emerald-300 dark:border-emerald-800/60 shadow-sm'
                  : isActive
                    ? 'bg-blue-50/90 dark:bg-blue-950/50 border-blue-400 dark:border-blue-700/60 shadow-md shadow-blue-500/10 ring-1 ring-blue-400/40'
                    : 'bg-slate-100/70 dark:bg-slate-900/40 border-slate-200/80 dark:border-slate-800/40 opacity-50'
              }`}
            >
              <div className={`p-2 rounded-xl shrink-0 ${
                isComplete
                  ...
                  : isActive
                    ? 'bg-blue-200 dark:bg-blue-900/60 text-blue-800 dark:text-blue-300'
      ...
      <div className="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden p-0.5 shadow-inner">
        <div
          className={`h-full rounded-full transition-all duration-700 ease-out ${verifyComplete ? 'bg-gradient-to-r from-emerald-500 to-green-500' : 'bg-gradient-to-r from-blue-500 via-cyan-400 to-blue-600'}`}
  ...
  // ═══ MAIN RENDER ══════════════════════════════════════
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-100/50 dark:from-blue-950/50 via-slate-50 dark:via-slate-950 to-slate-100 dark:to-slate-950 transition-colors duration-200">
      {/* Ambient Glow Effects */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -right-40 w-80 h-80 rounded-full bg-cyan-500/10 dark:bg-cyan-500/8 blur-[100px]" />
        <div className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full bg-blue-500/10 dark:bg-blue-500/8 blur-[100px]" />
        {step === STEP_DEMO_VERIFY && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-purple-500/10 dark:bg-purple-500/5 blur-[120px] animate-pulse" />
        )}
      </div>

      <div className={`w-full max-w-lg bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl relative overflow-hidden transition-all duration-300 text-slate-900 dark:text-slate-100 ${fadeIn ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
        {/* Inner glow accent */}
