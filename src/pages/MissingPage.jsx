...
export const MissingPage = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-colors duration-200">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="p-2.5 rounded-xl bg-purple-100 dark:bg-purple-950 text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-800 shadow-lg">
              <UserX className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-xl font-black text-slate-900 dark:text-white">MODULE 6: MISSING PERSONS REPOSITORY & AI CAMERA SCAN</h1>
      ...
      {/* Search Bar */}
      <div className="relative glass-panel p-2 rounded-2xl border border-slate-200 dark:border-slate-800 transition-colors duration-200">
        ...
        <input
          ...
          className="w-full bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-purple-500 placeholder-slate-400 dark:placeholder-slate-500"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map(person => (
          <div key={person.id} className="glass-panel glass-panel-hover p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4 flex flex-col justify-between transition-colors duration-200">
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="font-black text-xs text-purple-700 dark:text-purple-400">{person.id}</span>
              ...
              <div className="flex gap-4 items-center">
                ...
                />
                <div>
                  <h3 className="font-extrabold text-base text-slate-900 dark:text-white">{person.name}</h3>
                  <div className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
                    {person.age} Yrs • {person.gender}
                  </div>
                  <div className="text-[11px] text-cyan-700 dark:text-cyan-400 font-medium flex items-center gap-1 mt-1">
                    <MapPin className="w-3 h-3 text-red-500 dark:text-red-400" /> {person.last_seen}
                  </div>
                </div>
              </div>

              {person.description && (
                <div className="p-2.5 rounded-xl bg-slate-100/90 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-[11px] text-slate-600 dark:text-slate-400 mt-3">
                  "{person.description}"
                </div>
              )}

              {person.match_confidence && (
                <div className="p-2.5 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-800/80 text-emerald-800 dark:text-emerald-300 text-xs font-bold mt-2 flex items-center gap-1.5">
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2 text-xs text-slate-600 dark:text-slate-400">
              <div className="flex justify-between items-center">
                <span>Date: <b className="text-slate-800 dark:text-slate-300">{person.date_missing}</b></span>
                <span>Contact: <b className="text-slate-900 dark:text-slate-200">{person.contact}</b></span>
              </div>
              ...
              <button
                ...
                className={`w-full py-2.5 rounded-xl font-extrabold text-xs shadow flex items-center justify-center gap-2 border transition ${person.status === 'Match Found'
                  ? 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-emerald-700 dark:text-emerald-400 border-emerald-300 dark:border-emerald-500/30'
