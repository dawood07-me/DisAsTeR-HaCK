...
const RoleBadge = ({ role }) => {
  const configs = {
    'Citizen': { color: 'bg-blue-950/60 text-blue-300 border-blue-800/60', icon: UserPlus },
    'Rescue Team': { color: 'bg-red-950/60 text-red-300 border-red-800/60', icon: Shield },
    'Government Admin': { color: 'bg-amber-950/60 text-amber-300 border-amber-800/60', icon: ShieldCheck },
    'NGO / Volunteer': { color: 'bg-emerald-950/60 text-emerald-300 border-emerald-800/60', icon: HeartHandshake }
  };
...
const StatusBadge = ({ status }) => {
  if (status === 'Approved') {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-emerald-950/60 text-emerald-300 border border-emerald-800/60 text-[10px] font-extrabold">
  ...
  if (status === 'Rejected') {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-red-950/60 text-red-300 border border-red-800/60 text-[10px] font-extrabold">
        <XCircle className="w-3 h-3" /> Rejected
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-amber-950/60 text-amber-300 border border-amber-800/60 text-[10px] font-extrabold">
...
export const AdminPage = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div className="glass-panel p-6 rounded-2xl border border-amber-900/40 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-black text-white">GOVERNMENT ADMIN COMMAND CONTROL PANEL</h1>
        ...
        <div className="px-3 py-1.5 rounded-xl bg-amber-950/80 border border-amber-800 text-amber-300 font-bold text-xs flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <div className="text-[10px] uppercase font-bold text-slate-400">TOTAL SOS BEACONS LOGGED</div>
          <div className="text-3xl font-black text-white">{totalSOS}</div>
        ...
        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <div className="text-[10px] uppercase font-bold text-slate-400">ACTIVE RELIEF REQUESTS</div>
          <div className="text-3xl font-black text-emerald-400">{reliefRequests.length}</div>
          <div className="text-xs text-slate-400">NGO logistics dispatched</div>
        </div>

        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <div className="text-[10px] uppercase font-bold text-slate-400">SHELTER NETWORK HUBS</div>
          <div className="text-3xl font-black text-cyan-400">{shelters.length}</div>
          <div className="text-xs text-slate-400">24/7 Evacuation centers</div>
        </div>

        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <div className="text-[10px] uppercase font-bold text-slate-400">REGISTERED USERS</div>
          <div className="text-3xl font-black text-purple-400">{registeredUsers.length}</div>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <h3 className="font-extrabold text-base text-white flex items-center gap-2">
            <Users className="w-5 h-5 text-cyan-400" /> User Verification & Approval Panel
            {pendingCount > 0 && (
          ...
          {/* Filter Tabs */}
          <div className="flex gap-1 p-0.5 bg-slate-800/80 rounded-lg border border-slate-700">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {u.id_proof && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-bold border border-slate-700 flex items-center gap-1 transition"
                      >
                    ...
                    {u.verification_status === 'Approved' && (
                      <button
                        onClick={() => handleVerify(u.id, 'Rejected')}
                        disabled={actionLoading === u.id}
                        className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-red-900 text-slate-400 hover:text-red-300 text-[10px] font-extrabold flex items-center gap-1 transition disabled:opacity-50 border border-slate-700"
                      >
      ...
      {viewingProof && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewingProof(null)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-extrabold text-sm text-white flex items-center gap-2">
                <FileCheck className="w-4 h-4 text-cyan-400" /> ID Proof Document
              </h3>
              <button onClick={() => setViewingProof(null)} className="text-slate-400 hover:text-white">
                <XCircle className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-3">
              <div className="text-xs text-slate-400 space-y-1">
                <p><span className="font-bold text-slate-300">Name:</span> {viewingProof.name}</p>
                <p><span className="font-bold text-slate-300">Role:</span> {viewingProof.role}</p>
                <p><span className="font-bold text-slate-300">Email:</span> {viewingProof.email}</p>
              </div>
              {viewingProof.id_proof && (
                <img
                  src={viewingProof.id_proof}
                  alt="ID Proof Document"
                  className="w-full rounded-xl border border-slate-700 object-cover max-h-64"
                />
              )}
            </div>
          </div>
